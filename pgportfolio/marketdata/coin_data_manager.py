...
class CoinDataManager:
    ...
    def get_coin_features(self, start, end, period=300, features=('close',)):
        ...
        self._coins = coins
        #for coin in coins:
            #self._update_data(start, end, coin) #change 12
        ...
        #assert not np.any(np.isnan(data)), "Filling nan failed, unknown error." #change13
                ...
                if 'reversed_' in coin:
                    cursor.execute(
                        'INSERT INTO History VALUES (?,?,?,?,?,?,?,?,?)',
                         ...
                         #1.0 / weightedAverage))
                        (c['date'], coin, 1.0 / float(c['low']), 1.0 / float(c['high']), #change10
                         1.0 / float(c['open']),
                    ...
                    cursor.execute(
                        #'INSERT INTO History VALUES (?,?,?,?,?,?,?,?,?)',
                        'INSERT OR REPLACE INTO History VALUES (?,?,?,?,?,?,?,?,?)', #change11
