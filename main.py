...
from pytorch_lightning.loggers import mlflow_logger #change 1
...
def main():
    ...
    if options.mode == "train":
        ...
        checkpoint_callback = ModelCheckpoint(
            ...
            filepath = options.working_dir + "/model", #change 2
        ...
        #)
        t_logger = mlflow_logger.MLFlowLogger(options.working_dir + "/log/tensorboard_log")
        trainer = pl.Trainer(
            gpus=0 if options.device == "cpu" else options.device, #DIFF!!!
            #callbacks=[checkpoint_callback, early_stopping],
            #logger=[t_logger],
            #limit_train_batches=1000,
            #max_steps=config["training"]["steps"]
            logger=True, #change3
            checkpoint_callback=True, early_stop_callback=True, default_save_path=[t_logger],
        ...
        model = TraderTrainer(config,
                        ...
                        online=options.offline, #change4
